...
// On window load, invoke loadNames function
window.onload = function () {
  loadNames();
};

// Function to fetch and process user data from Supabase
async function loadNames() {
  try {
    ...
    // Process data to count occurrences of each name
    const nameCount = data.reduce((acc, user) => {
      acc[user.name] = (acc[user.name] || 0) + 1;
      return acc;
    }, {});

    // Sort names by frequency, then alphabetically
...
// Function to render the names data into HTML
function renderNames(sortedNamesData) {
  const namesContainer = document.getElementById("names-container");
  namesContainer.innerHTML = ""; // Clear loading text when data is fetched
  ...
  // Loop through each name and display it in a user item
  sortedNamesData.forEach(([name, count], index) => {
    const userItem = document.createElement("div");
    userItem.className = "user-item";

    // Apply classes for top 3 ranked users
    if (index === 0) {
...
// Fetch all ideas for a specific user
...
// Render fetched ideas into the modal
